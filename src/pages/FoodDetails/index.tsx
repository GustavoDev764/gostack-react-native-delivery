...
const FoodDetails: React.FC = () => {
  ...
  useEffect(() => {
    async function loadFood(): Promise<void> {
      ...
      setExtras(
        response.data.extras.map((e: Omit<Extra, 'quantity'>) => ({
          ...e,
          quantity: 0,
        })),
      );
  ...
  const cartTotal = useMemo(() => {
    const extraTotal = extras.reduce((accumlator, extra) => {
      return accumlator + extra.quantity * extra.value;
    }, 0);
  ...
  async function handleFinishOrder(): Promise<void> {
    try {
      const newFood = { ...food, extras };
      api.post('orders', newFood);
      Alert.alert(
        'Message',
